...
#   TransformerKernels.∇score(m::MyMod, ∂m, s, b, h, q, kv, s̄) -> s̄_in
# (No generic fallback ON PURPOSE — see the `pair_feature` docstring.)

"""
    grad_shadow(score_mod)

Build the gradient shadow of a score mod: the same struct with every array
field replaced by a zeroed copy, recursing through composed mods. Pass it as
`∂score_mod` to [`∇flex_attention!`](@ref) and read the accumulated gradients
from its fields afterwards. Gradients accumulate in place, so a shadow is
valid for ONE backward call — rebuild (or re-zero) to reuse.
"""
function grad_shadow(m::ScoreMod)
    T = typeof(m)
    T.name.wrapper(ntuple(i -> _shadow(getfield(m, i)), Val(fieldcount(T)))...)
end
_shadow(x::AbstractArray) = fill!(similar(x), 0)
_shadow(x::ScoreMod) = grad_shadow(x)
_shadow(x) = x
