...
function mha_decode_split_fwd(
    ...
    for j in j_lo:j_hi
        ...
        s_qk = muladd((k)ᵀ → Tc, q → Tc, zeros(Tacc, (TILE_N, GROUP)))
        s_qk = s_qk * Tacc(qk_scale)
...
function mha_decode_combine(
    ...
    for s in 1i32:N_SPLITS
        ...
        finite = m_new .> -Inf32
        alpha = ifelse.(finite, exp.(m_i .- m_new), 1f0)
        gamma = ifelse.(finite, exp.(m_s .- m_new), 0f0)

        l_i = l_i .* alpha .+ l_s .* gamma
        acc = acc .* Tacc.(alpha) .+ Tacc.(l_s .* gamma) .* (o_s → Tacc)
        m_i = m_new
    end
...
function decode_attention!(O,
    ...
    O_partial = similar(O, accumulate, (Dv, Heads, Batch, n_splits))
