...
function TransformerKernels.pair_feature(op::RBFPairOp, qv::NTuple{3}, kv::NTuple{3})
...
function flex_bwd_ref(Ō, Q, K, V; score = nothing, dscore = nothing,
    ...
    for b in 1:B, h in 1:H
        hk = cld(h, g)
        S0 = (transpose(K[:, :, hk, b]) * Q[:, :, h, b]) .* scale
        S = copy(S0)
        ...
        K̄[:, :, hk, b] .+= Q[:, :, h, b] * transpose(S̄0) .* scale
...
@testset "∇flex_attention!" begin
    ...
    function run_flex_bwd(Q, K, V; score_mod = NoOpScore(), mask_mod = FullMask(),
        ...
        dM = similar(dQ, Float32, Mq, H, B)
        dL = similar(dM)
        flex_attention!(dO, dQ, dK, dV; score_mod, mask_mod, M = dM, L = dL, kwargs...)
        Ō = randn(rng, Float64, Dv, Mq, H, B)
        dŌ = CuArray(Float32.(Ō))
        dQ̄, dK̄, dV̄ = similar(dQ), similar(dK), similar(dV)
        ∇flex_attention!(dQ̄, dK̄, dV̄, dŌ, dQ, dK, dV, dO, dM, dL;
                         score_mod, mask_mod, ∂score_mod, kwargs...)
    ...
    @testset "pair feature grads (∂proj + features)" begin
        ...
        dq, dk = TransformerKernels.∇pair_feature(op, qv, kv, dphi)
        ϵ = 1e-5
        for f in 1:3
            bump = ntuple(i -> i == f ? ϵ : 0.0, 3)
            num_q = sum(dphi .* (TransformerKernels.pair_feature(op, qv .+ bump, kv) .-
                                 TransformerKernels.pair_feature(op, qv .- bump, kv))) / 2ϵ
            num_k = sum(dphi .* (TransformerKernels.pair_feature(op, qv, kv .+ bump) .-
                                 TransformerKernels.pair_feature(op, qv, kv .- bump))) / 2ϵ
        ...
        score = (s0,b,h,q,kv) -> begin
            qv, kvv = pair(q, kv, b)
            s0 + sum(proj[h, :] .* TransformerKernels.pair_feature(op, qv, kvv))
        end
        dscore = (s0,b,h,q,kv,s̄) -> begin
            qv, kvv = pair(q, kv, b)
            phi = TransformerKernels.pair_feature(op, qv, kvv)
            proj_ref[h, :] .+= s̄ .* phi
            dphi = ntuple(p -> proj[h, p] * s̄, 3)
            dq, dk = TransformerKernels.∇pair_feature(op, qv, kvv, dphi)
            for f in 1:3
                qf_ref[f, q+1, b] += dq[f]
